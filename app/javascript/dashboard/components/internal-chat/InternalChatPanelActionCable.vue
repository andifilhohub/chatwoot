...
const generalChat = computed(() => {
  ...
  if (generalRoom) {
    return {
      ...
      description:
        generalRoom.description || t('INTERNAL_CHAT.ROOMS.TYPE.GENERAL'),
...
const currentChat = computed(() => {
  ...
  if (selectedChatType.value === 'team' && selectedChatId.value) {
    return (
      teamChats.value.find(team => team.id === selectedChatId.value) || null
    );
  }
  if (selectedChatType.value === 'direct' && selectedChatId.value) {
    return (
      directMessages.value.find(agent => agent.id === selectedChatId.value) ||
      null
    );
  }
  return null;
...
<template>
  ...
  >
    ...
    <span
      class="hidden bg-green-500 bg-yellow-500 bg-gray-400"
      aria-hidden="true"
    />
    <div
    ...
    >
      <!-- Sidebar -->
      <div
        class="w-80 min-w-[320px] max-w-[320px] bg-n-solid-2 border-r border-n-weak flex flex-col"
      >
        <div
          class="px-6 py-4 border-b border-n-weak h-[84px] flex items-center"
        >
          <div>
        ...
        <div v-if="loading" class="flex-1 flex items-center justify-center">
          <div class="text-center">
            <div
              class="animate-spin rounded-full h-8 w-8 border-b-2 border-woot-600 mx-auto"
            />
            <p class="text-sm text-n-slate-10 mt-2">
              {{ $t('INTERNAL_CHAT.STATE.LOADING_ROOMS') }}
            </p>
          </div>
        </div>
        ...
        <div v-else class="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div class="space-y-2">
            <h4
              class="text-xs font-semibold text-n-slate-10 uppercase tracking-wide px-2"
            >
            ...
            >
              <div
                class="w-10 h-10 bg-green-500 rounded-full flex items-center justify-center text-white text-sm font-medium mr-3 shadow-inner"
              >
                #
              </div>
              <div class="flex-1">
              ...
              <div
                v-if="selectedChatType !== 'general'"
                class="w-2 h-2 bg-green-400 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"
              />
            </div>
          </div>
          ...
          <div class="space-y-2">
            ...
            <div v-if="teamChats.length > 0">
              <div v-for="team in teamChats" :key="team.id">
                ...
                >
                  <div
                    class="w-10 h-10 bg-blue-500 rounded-full flex items-center justify-center text-white text-sm font-medium mr-3 shadow-inner"
                  >
                  ...
                  <div
                    v-if="
                      selectedChatType !== 'team' || selectedChatId !== team.id
                    "
                    class="w-2 h-2 bg-blue-400 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"
                  />
                </div>
              </div>
            </div>
            <div
              v-else
              class="px-3 py-2 text-center text-n-slate-10 text-sm italic"
            >
              ...
              <div v-for="agent in directMessages" :key="agent.id">
                ...
                >
                  <div class="relative mr-3">
                    ...
                    <div
                      v-else
                      class="w-10 h-10 rounded-full bg-woot-600 flex items-center justify-center text-white font-semibold shadow-md"
                    >
                  ...
                  <div
                    ...
                    :class="[getStatusColor(agent.availability_status)]"
                  />
                </div>
              </div>
            </div>
      ...
      <div class="flex-1 flex flex-col">
        ...
        <div
          class="flex-none flex items-center justify-between p-4 border-b border-n-weak h-[84px]"
        >
          <div v-if="currentChat" class="flex items-center space-x-3">
            <div class="flex-shrink-0">
              <div v-if="selectedChatType === 'direct'" class="relative">
                <div
                  v-if="currentChat.avatar_url"
                  class="w-10 h-10 rounded-full overflow-hidden shadow-md"
                >
                  <img
                ...
                <div
                  ...
                  :class="[getStatusColor(currentChat.availability_status)]"
                />
              </div>
              <div
                v-else
                class="w-10 h-10 rounded-full bg-woot-600 flex items-center justify-center shadow-md"
              >
                <svg
            ...
            <div class="min-w-0 flex-1">
              ...
              <p
                v-if="selectedChatType === 'direct'"
                class="text-sm text-n-slate-10"
              >
                {{ getStatusText(currentChat.availability_status) }}
                <span
                  v-if="
                    currentChat.availability_status !== 'online' &&
                    currentChat.last_seen_at
                  "
                  class="ml-1"
                >
              ...
              <p
                v-else-if="selectedChatType === 'team'"
                class="text-sm text-n-slate-10"
              >
                {{ currentChat?.member_count || 0 }}
                {{
                  (currentChat?.member_count || 0) === 1 ? 'member' : 'members'
                }}
              </p>
        ...
        <div
          ...
          class="flex-1 overflow-y-scroll p-4 space-y-4"
        ...
        >
          ...
          >
            <div class="text-center">
              <div
                class="w-16 h-16 mx-auto mb-4 bg-n-solid-3 rounded-full flex items-center justify-center"
              >
                <svg
              ...
              <h3 class="text-lg font-medium text-n-slate-12 mb-2">
                {{ $t('INTERNAL_CHAT.ROOMS.NO_ROOM_SELECTED') }}
              </h3>
          ...
          >
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                ...
                <div
                  class="bg-n-solid-3 rounded-2xl rounded-tl-md px-4 py-3 shadow-sm"
                >
            ...
            <div v-else class="flex space-x-3 max-w-[70%] flex-row-reverse">
              <div class="flex-shrink-0">
                ...
                <div
                  v-else
                  class="w-8 h-8 rounded-full bg-woot-600 flex items-center justify-center text-white text-sm font-medium shadow-sm"
                >
              ...
              <div class="flex-1 min-w-0">
                ...
                <div
                  class="bg-woot-600 rounded-2xl rounded-tr-md px-4 py-3 shadow-sm"
                >
        ...
        <div class="flex-none border-t border-n-weak p-4">
          <form
            ...
            @submit.prevent="sendMessageAction"
          >
            <div class="flex-1">
              <textarea
